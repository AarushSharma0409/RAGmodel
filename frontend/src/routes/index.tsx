...
import { ChevronDown, Upload, X, Send, FileText, ChevronRight, Trash2, Loader2 } from "lucide-react";
...
// genId() requires HTTPS — use this fallback for local dev
function genId() {
...
function UploadSection() {
  ...
  const deleteDocument = useCallback(async (doc: Doc) => {
    setDocumentActionError(null);
    setDeletingDocs((current) => new Set(current).add(doc.id));
    try {
      const res = await fetch(`${API_BASE}/documents/${encodeURIComponent(doc.name)}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error("Delete failed");
      setDocs((current) => current.filter((item) => item.id !== doc.id));
    } catch {
      setDocumentActionError(`Could not delete ${doc.name}.`);
    } finally {
  ...
  const clearDocuments = useCallback(async () => {
    if (!docs.length || clearing) return;
    const confirmed = window.confirm("Clear all uploaded documents from DocMind?");
    if (!confirmed) return;
    ...
    try {
      ...
      setDocumentActionError("Could not clear the vector database.");
    } finally {
      setClearing(false);
    }
  }, [clearing, docs.length]);
  ...
  const uploadFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      setDocs((d) => [
        ...d.filter((existing) => existing.name !== file.name),
        { id, name: file.name, size: file.size },
      ]);
    } catch {
      setDocumentActionError(`Could not upload ${file.name}.`);
    } finally {
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          {docs.length > 0 && (
            <button
              ...
              className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-rose-400/20 bg-rose-500/10 text-rose-300 transition-colors hover:border-rose-300/40 hover:bg-rose-500/20 disabled:cursor-not-allowed disabled:opacity-50"
            >
            ...
            {docs.map((doc) => (
              ...
              >
                ...
                <button
                  ...
                  disabled={deletingDocs.has(doc.id) || clearing}
                  title={`Delete ${doc.name}`}
                  className="rounded-lg p-2 text-foreground/60 transition-colors hover:bg-slate-700 hover:text-rose-400 disabled:cursor-not-allowed disabled:opacity-50"
                >
...
function ChatSection() {
  ...
  const send = async () => {
    ...
    try {
      ...
      // Backend returns: {answer, citations, confidence: {level, reason}, route}
      const confLevel = data.confidence?.level ?? "medium";
      const conf: "high" | "medium" | "low" =
        confLevel === "high" ? "high" : confLevel === "low" ? "low" : "medium";
      // Map backend citations {source_file, page_number, locator_type, excerpt}
      // to frontend shape {source, page}
